...
REFERENCE_PUBLICATIONS: dict[str, ReferencePublication] = {
    "mace_mp0": ReferencePublication(
        ...
        role="Published MACE-MP-0/MPA-0 uncertainty envelope; OC157 MAD.",
    ),
...
class AdsorbMLReference:
    ...
    functional: str
    dispersion: str
    sign_convention: str = "E_ads = E_slab+ads - E_clean_slab - E_gas_ads; negative exothermic"
...
def strict_adsorbml_references() -> dict[tuple[str, str], AdsorbMLReference]:
    ...
    return {
        key: ref
        for key, ref in ADSORBML_REFERENCES.items()
        if ref.strict_for_parity
    }


# MACE-MPA-0+D3 OC157 molecule-surface relative-energy MAD from the
# arXiv v3 foundation-model supplement. This is the relevant uncertainty
# scale for ranking molecule-on-surface configurations, not chemical accuracy.
