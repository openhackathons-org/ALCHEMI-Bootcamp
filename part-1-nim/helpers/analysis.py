...
from .references import AdsorbMLReference, MACE_MPA0_OC157_MAD_EV
...
def _adsorbate_integrity_status(
    ...
    if len(final_ads) > 1 and len(initial_ads) == len(final_ads):
        for i in range(len(final_ads) - 1):
            d_initial = float(np.linalg.norm(
                initial_ads.positions[i] - initial_ads.positions[i + 1]
            ))
            d_final = float(np.linalg.norm(
                final_ads.positions[i] - final_ads.positions[i + 1]
            ))
            if d_initial > 0.1 and d_final > 1.5 * d_initial:
                return "dissociated"
...
def build_pair_results_table(
    ...
    if len(configs) != len(opt_results):
        raise ValueError(
            "BGR result count does not match configuration count: "
    ...
    for config, opt in zip(configs, opt_results):
        ...
        geometry_status = _adsorbate_integrity_status(
            config.atoms, final_atoms, slab_atom_count=slab_n
        )
        force_ok = bool(np.isnan(max_force) or max_force <= 0.2)
...
def summarize_pair_validation(
    ...
    mad_ev: float = MACE_MPA0_OC157_MAD_EV,
) -> pd.DataFrame:
    ...
    for (host, adsorbate), df in pair_results.items():
        ...
        if candidates.empty:
            winner = df.loc[df["E_bind (eV)"].idxmin()]
            winner_is_reliable = False
        else:
            winner = candidates.loc[candidates["E_bind (eV)"].idxmin()]
        ...
        if winner_is_reliable:
            status, delta, scaled = _validation_status(
                float(winner["E_bind (eV)"]), ref, site_match, mad_ev
            )
        ...
        rows.append(ValidationResult(
            ...
            E_MACE_eV=round(float(winner["E_bind (eV)"]), 3),
