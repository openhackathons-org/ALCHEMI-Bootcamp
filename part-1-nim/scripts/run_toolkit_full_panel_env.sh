...
export PYTHONPATH="${repo_root}/part-1-nim${PYTHONPATH:+:${PYTHONPATH}}"
export ADSORPTION_BACKEND="${ADSORPTION_BACKEND:-toolkit}"
...
export XDG_CACHE_HOME="${XDG_CACHE_HOME:-${repo_root}/.xdg-cache}"
export TORCH_HOME="${TORCH_HOME:-${repo_root}/.torch-cache}"
export HF_HOME="${HF_HOME:-${repo_root}/.hf-cache}"
export WARP_CACHE_PATH="${WARP_CACHE_PATH:-${repo_root}/.warp-cache}"
...
exec "${python_bin}" part-1-nim/scripts/run_toolkit_full_panel.py "$@"
