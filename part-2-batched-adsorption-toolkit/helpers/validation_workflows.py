...
from .cache_registry import ACCURACY_NH3_RANKING_STEM, ACCURACY_TRAJECTORY_STEM
...
def make_oc20dense_validation_context(
    *,
    ...
    trajectory_root = accuracy_output_dir / ACCURACY_TRAJECTORY_STEM
    nh3_ranking_root = accuracy_output_dir / ACCURACY_NH3_RANKING_STEM
...
def describe_oc20dense_system(
    ...
    slab_formula = _slab_formula_for_system(
        data_root, system_id=str(system_id), sid=sid, meta=meta
    )
    material_label = slab_formula or str(meta.get("mpid", "OC20Dense slab"))
    material_label_html = (
        _formula_html_subscripts(material_label) if slab_formula else material_label
    )
    surface_label_plain = (
...
def _slab_formula_for_system(
    ...
    *,
    ...
    candidates.append(
        data_root / "selected_trajectories" / "surfaces" / f"{system_id}_surface.traj"
    )
...
def prepare_nh3_ranking_reference_source(
    *,
    ...
    target_by_config = {
        str(config_id): float(energy) for config_id, energy in targets[str(system_id)]
    }
    ...
    for sid, meta in sorted(
        mapping.items(), key=lambda item: str(item[1]["config_id"])
    ):
        ...
        trajectory_path = (
            data_root / "selected_trajectories" / "adslab" / f"{label}.traj"
        )
        ...
        [
            {
                "system_id": str(system_id),
                "n_configs": len(rows),
                "dft_best_config": min(rows, key=lambda row: row["dft_rank"])[
                    "config_id"
                ],
                "sp_best_config": min(rows, key=lambda row: row["dft_rank"])[
                    "config_id"
                ],
                "ml_best_config": min(rows, key=lambda row: row["dft_rank"])[
                    "config_id"
                ],
            }
        ]
...
def run_or_load_oc20dense_validation(
    ...
    *,
    ...
    all_steps = [("relaxation check", step) for step in trajectory_steps] + [
    ...
    try:
        with log_path.open("w", encoding="utf-8") as log_handle:
            for index, (piece, step) in enumerate(all_steps, start=1):
                ...
                if progress is not None:
                    progress.update(
                        done=index, message=f"{piece}: {step.label} complete"
                    )
    finally:
...
def run_nh3_single_point_batches(
    ...
    *,
    ...
    if chunk_timing_path.exists():
        ...
        occupancy = ", ".join(
            str(value) for value in chunk_timings["n_configs"].astype(int)
        )
        if display_fn is not None and markdown_cls is not None:
            display_fn(
                markdown_cls(
...
def _adsorbate_unwrapped_local_positions(
    atoms, adsorbate_mask: np.ndarray
) -> np.ndarray:
...
def _adsorbate_unwrapped_positions_in_cell(
    atoms, adsorbate_mask: np.ndarray
) -> np.ndarray:
...
def _top_down_adsorbate_site_camera(
    ...
    *,
    ...
    if target == "extent":
        ...
        center += lateral_1 * float(
            (lateral_1_values.min() + lateral_1_values.max()) / 2.0
        )
        center += lateral_2 * float(
            (lateral_2_values.min() + lateral_2_values.max()) / 2.0
        )
...
def _assert_single_nh3_structure(atoms, *, source: str) -> None:
    ...
    if counts.get("O", 0):
        raise ValueError(
            f"Expected NH3-only adsorbate, found adsorbate O atoms in {source}."
        )
...
def validation_trajectory_artifact_paths(
    ...
    if missing:
        raise FileNotFoundError(
            "Missing validation trajectory artifacts: " + "; ".join(missing)
        )
...
def show_nh3_ranking_results(
    ...
    *,
    ...
    ranking_table = rows[ranking_columns].rename(
        columns={
            "dft_rank": "DFT rank",
            "dft_rank1_relative_energy_eV": "DFT gap from DFT-best (eV)",
            "mace_dft_rank1_relative_total_energy_eV": "MACE gap from DFT-best geometry (eV)",
...
def show_nh3_all_geometry_grid(
    ...
    *,
    ...
    if missing:
        raise FileNotFoundError(
            "Missing NH3 geometry widget structures: "
            + "; ".join(missing[:5])
...
def show_nh3_all_geometry_render_grid(
    ...
    *,
    ...
    label_style_token = "_labelv2"
    overview_path = visual_root / (
        f"nh3_92_geometry_scan_{focus_token}{annotation_token}{label_style_token}"
    ...
    if missing:
        raise FileNotFoundError(
            "Missing NH3 geometry render structures: "
            + "; ".join(missing[:5])
            + (f"; ... {len(missing)} total" if len(missing) > 5 else "")
        )
            ...
            [
                {
                    ...
                    "annotation_scale": annotation_scale,
                    "label_style": label_style_token.lstrip("_"),
...
def _compose_nh3_render_overview(
    ...
    *,
    ...
    gap = 16
    margin = 24
    title_height = int(round(112 * scale))
    label_height = int(round(128 * scale))
    title_offset = int(round(44 * scale))
    subtitle_offset = int(round(76 * scale))
    label_pad_x = int(round(14 * scale))
    label_pad_y = int(round(12 * scale))
    label_line_step = int(round(27 * scale))
    ...
    font_title = _pil_font(int(round(32 * scale)), bold=True)
    font_subtitle = _pil_font(int(round(19 * scale)))
    font_label = _pil_font(int(round(17 * scale)))
    font_label_bold = _pil_font(int(round(19 * scale)), bold=True)

    draw.text(
        (margin, margin),
        "NH3 92-configuration visual scan",
        fill=(20, 24, 31),
        font=font_title,
    )
    draw.text(
...
def show_nh3_geometry_widgets(
    ...
    *,
    ...
    for left, right in combinations(highlight_paths.itertuples(index=False), 2):
        ...
        left_pos, left_cell, left_pbc = adsorbate_geometry(
            left.resolved_dft_final_structure_path
        )
    ...
    for row in highlight_paths.sort_values("dft_rank").itertuples(index=False):
        ...
        _assert_single_nh3_structure(
            atoms, source=row.resolved_dft_final_structure_path
        )
        if show_cell:
    ...
    display_fn(
        markdown_cls("#### Geometry spread among highlighted NH<sub>3</sub> ranks")
    )
...
def show_validation_model_tradeoff(
    *,
    ...
    display_fn(
        markdown_cls("#### Model choice is a license, cost, and accuracy trade-off")
    )
    display_fn(model_policy.style.hide(axis="index"))
    display_fn(
        markdown_cls("#### Measured open-model baseline for this validation slice")
    )
