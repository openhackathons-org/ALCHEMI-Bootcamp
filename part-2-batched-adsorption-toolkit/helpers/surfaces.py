...
def build_adsorbate(species: AdsorbateSpecies) -> ase.Atoms:
    ...
    that ``ase_to_atomic_data`` propagates it to the Toolkit relaxation payload.
    The MACE-family model used here does not use this field, but it is set
    for scientific documentation and future compatibility with DFT-based
    models.
...
def classify_relaxation(
    ...
    relaxed_result : OptimizationResult
        Relaxation result with coord, energy, forces, converged.
    slab_n_atoms : int
    ...
    converged : bool
        Whether the Toolkit optimiser reported convergence.
    n_steps : int
...
def place_adsorbate_at_site(
    ...
    *,
    ...
    """Place an adsorbate above a surface site along the slab normal.

    Optionally rotates the molecule about z, lifts it ``height_A`` above
    ``site_position`` along the outward surface normal, and returns slab+ads.
    Geometry mechanics for the notebook's starting-grid builder.
    """
    ads = adsorbate_atoms.copy()
    if abs(rotation_deg) > 1e-6:
        ads.rotate(rotation_deg, "z", center=(0.0, 0.0, 0.0))
    ads.translate(site_position + height_A * _surface_normal(slab))
