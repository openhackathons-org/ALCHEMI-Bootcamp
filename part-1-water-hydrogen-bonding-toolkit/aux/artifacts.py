...
def write_water_run_manifest(
    ...
    *,
    ...
    The caller owns the scientific content of ``provenance``, ``settings``,
    and ``gates``. This function records those values without inventing a
    timestamp or status, then inventories every file beneath the run-specific
    output directory. The manifest excludes only itself, so repeated calls
    with unchanged inputs and artifacts are byte-identical.
    """
    ...
    manifest = _json_ready(
        {
            "schema": "alchemi.water-ir-run.v1",
...
def save_ir_trajectory(
    ...
    """Save the complete raw trajectory before scientific comparison gates."""
...
def graph_atoms_from_batch(batch: Any, graph_index: int, label: str) -> Atoms:
