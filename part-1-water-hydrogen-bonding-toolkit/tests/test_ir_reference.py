...
import aux.reference as reference_api
from aux.reference import core as ir_reference
...
class ArtifactAndSpectrumTests(unittest.TestCase):
    ...
    def test_failed_scientific_gate_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            manifest_path = _write_artifact(
                root,
            ...
            with self.assertRaisesRegex(
                ir_reference.IRArtifactError, "consumer gate"
            ):
                ir_reference.load_psi4_b973c_ir_artifact(manifest_path)
