#!/usr/bin/env python3
"""Apply the measured NCI/AIMNet Part 3 design to the research notebook.
...
NOTEBOOK = Path("part-3-toolkit-foundations/alchemi-toolkit-foundations.ipynb")
...
def main() -> None:
    ...
    cells["title"]["source"] = source(
        r"""
# Part 3 — ALCHEMI Toolkit: atoms → batched GPU workflows
...
def find_repo_root(start: Path | None = None) -> Path:
    ...
    for candidate in (start, *start.parents):
        if (candidate / "build" / "requirements.txt").is_file() and (
            candidate / "part-3-toolkit-foundations"
        ).is_dir():
            return candidate
...
ROOT = find_repo_root()
NOTEBOOK_DIR = ROOT / "part-3-toolkit-foundations"
DATA_FILE = NOTEBOOK_DIR / "data" / "nci-atlas-curves.csv.gz"
OUTPUT_DIR = NOTEBOOK_DIR / "outputs"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

TOOLKIT_CORE_COMMIT = "b770ee6963fd2f6137891e408c370012751918e2"
TOOLKIT_OPS_COMMIT = "c6fbe652315e0cebd4f57a6a25f626258f0dbbfd"
