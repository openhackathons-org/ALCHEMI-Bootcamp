"""Template for adding literature molecules WITHOUT structure files.
...
Build each geometry as an ``ase.Atoms`` in code — inline coordinates from a cited
source, ``ase.build.molecule`` for G2 species, or a pymatgen ``Molecule`` — then
register it under the same ``candidate_id`` you used in the manifest. The notebook
MLIP-relaxes every molecule before any energy is used, so an idealized starting
geometry is fine.
"""

from ase import Atoms

from challenge_utils.molecules import register_molecule

# Example: register an inline geometry (replace with your literature molecule).
# register_molecule(
#     "my_literature_additive",
#     Atoms(
