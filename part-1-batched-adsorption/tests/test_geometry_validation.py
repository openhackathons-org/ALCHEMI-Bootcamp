...
SCREEN_HOST_BUILDERS = {
    "Cu(111)": lambda: build_cu111_slab(
        min_slab_size=8.0, min_vacuum_size=15.0, supercell=(3, 3, 1)
    ),
    "Cu(100)": lambda: build_cu100_slab(
        min_slab_size=8.0, min_vacuum_size=15.0, supercell=(3, 3, 1)
    ),
    "Cu(110)": lambda: build_cu110_slab(
        min_slab_size=8.0, min_vacuum_size=15.0, supercell=(3, 3, 1)
    ),
    "TiO2(110)": lambda: build_tio2_110_slab(
        min_slab_size=8.0, min_vacuum_size=15.0, supercell=(2, 2, 1)
    ),
    "TiO2(100)": lambda: build_tio2_100_slab(
        min_slab_size=8.0, min_vacuum_size=15.0, supercell=(2, 2, 1)
    ),
    "TiO2(101)": lambda: build_tio2_101_slab(
        min_slab_size=8.0, min_vacuum_size=15.0, supercell=(2, 2, 1)
    ),
    "TiN(001)": lambda: build_tin_001_slab(
        min_slab_size=8.0, min_vacuum_size=15.0, supercell=(2, 2, 1)
    ),
    "TiN(110)": lambda: build_tin_110_slab(
        min_slab_size=8.0, min_vacuum_size=15.0, supercell=(2, 2, 1)
    ),
    "TiN(210)": lambda: build_tin_210_slab(
        min_slab_size=8.0, min_vacuum_size=15.0, supercell=(2, 2, 1)
    ),
}
        ...
        for adsorbate in SCREEN_ADSORBATES:
            configs.extend(
                build_config_grid(
                    ...
                    orientations_filter=list(
                        SCREEN_ORIENTATIONS_BY_ADSORBATE[adsorbate]
                    ),
                    rotations_deg=(0.0,),
                    heights_A=(2.5,),
                )
...
def test_new_surface_screen_slabs_have_registered_sites(
    host_name, builder, expected_sites
):
...
def test_surface_screen_builds_216_programmatic_starting_structures():
    ...
    for config in configs:
        counts[(config.host, config.adsorbate)] = (
            counts.get((config.host, config.adsorbate), 0) + 1
        )
...
def test_adsorbate_orientation_places_expected_atom_lowest(
    ...
    for orient, expected_symbols in zip(orientations, lowest_symbols):
        ...
        if name == "CH3OH" and orient == "methyl-down":
            ...
            assert (
                atoms.positions[symbols.index("O"), 2]
                > atoms.positions[symbols.index("C"), 2]
            )
