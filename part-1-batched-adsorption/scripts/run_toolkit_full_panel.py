...
HEIGHTS = (2.2,)
OUTPUT_DIR = PART1 / "outputs" / "live_runs" / "manual" / "full_panel_toolkit"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no-trajectories",
        action="store_true",
        help="Disable per-step extxyz trajectory and energy/force log output.",
    )
...
def _artifact_paths(label: str, *, trajectory_subdir: str | None = None) -> dict[str, Path]:
    safe_label = _safe(label)
    trajectory_dir = TRAJECTORY_DIR
    log_dir = TRAJECTORY_LOG_DIR
    if trajectory_subdir:
        trajectory_dir = trajectory_dir / trajectory_subdir
        log_dir = log_dir / trajectory_subdir
    return {
        "initial": STRUCTURE_DIR / "initial" / f"{safe_label}.extxyz",
        "relaxed": STRUCTURE_DIR / "relaxed" / f"{safe_label}.extxyz",
        "trajectory": trajectory_dir / f"{safe_label}.extxyz",
        "log": log_dir / f"{safe_label}.csv",
    }
...
def _relax_one(
    ...
    *,
    ...
    trajectory_subdir: str | None = None,
):
    start = time.perf_counter()
    payload = ase_to_atomic_data(atoms, structure_id=label, active_mask=active_mask)
    if write_trajectory:
        paths = _artifact_paths(label, trajectory_subdir=trajectory_subdir)
...
def _ensure_output_dirs() -> None:
    for directory in (
        ...
        STRUCTURE_DIR / "gas",
        TRAJECTORY_DIR,
        TRAJECTORY_DIR / "clean",
        TRAJECTORY_DIR / "gas",
        TRAJECTORY_DIR / "reruns",
        TRAJECTORY_LOG_DIR,
        TRAJECTORY_LOG_DIR / "clean",
        TRAJECTORY_LOG_DIR / "gas",
        TRAJECTORY_LOG_DIR / "reruns",
    ):
...
def _relax_configs_in_chunks(
    ...
    *,
    ...
    for start_idx in range(0, len(configs), chunk_size):
        ...
        trajectory_paths = [
            _artifact_paths(config.label)["trajectory"] for config in chunk
        ]
        log_paths = [_artifact_paths(config.label)["log"] for config in chunk]
        trajectories_available = (
            not write_trajectory
            or all(path.exists() for path in trajectory_paths + log_paths)
        )
        if paths["raw"].exists() and paths["csv"].exists() and trajectories_available and not force:
            ...
            chunk_df["initial_structure_path"] = [
                str(_artifact_paths(config.label)["initial"]) for config in chunk
            ]
            chunk_df["relaxed_structure_path"] = [
                str(_artifact_paths(config.label)["relaxed"]) for config in chunk
            ]
            chunk_df["trajectory_path"] = [str(path) for path in trajectory_paths]
            chunk_df["trajectory_log_path"] = [str(path) for path in log_paths]
            ...
            for config, result in zip(chunk, chunk_results):
                _write_initial_atoms(
                    _artifact_paths(config.label)["initial"],
                    config.atoms,
                    label=config.label,
                )
                _write_result(
                    RAW_DIR / f"{_safe(config.label)}.json",
                    result,
                )
                _write_result_atoms(
                    _artifact_paths(config.label)["relaxed"],
                    result,
                    label=config.label,
                )
        ...
        chunk_rows.append(
            {
                ...
                "trajectory_log_dir": str(TRAJECTORY_LOG_DIR),
            }
        )
        print(
...
def _write_report(
    *,
    ...
    e_col = adsorption_energy_column(full_df)
    minima = (
        reliable.sort_values(e_col)
    ...
    lines = [
        ...
        "Backend: ALCHEMI Toolkit",
        (
            "Model: "
            + (
                f"{metadata.get('toolkit_checkpoint', 'medium-mpa-0')} "
                f"(head={metadata.get('toolkit_head')})"
                if metadata.get("toolkit_head")
                else metadata.get("toolkit_checkpoint", "medium-mpa-0")
            )
            + "; D3(BJ) disabled"
        ),
        "",
        ...
        "so these numbers are not strict D3-corrected or DFT-level parity values.",
        "",
        ...
        f"For orientation only, a literature OC157 relative-energy MAD guide is {LITERATURE_OC157_MAD_GUIDE_EV:.2f} eV; "
        "it is not an error bar for this MACE-MPA-0, D3-disabled report. "
        "Strict comparison still requires exact "
        "row-level DFT metadata: slab, coverage, frozen layers, functional, "
        "dispersion, sign convention, and backend/D3 settings.",
        "",
        "## Chunk Timings",
        "",
...
def main() -> int:
    ...
    for host, slab in hosts.items():
        ...
        clean_label = f"full_clean_{safe_host}"
        raw_path = RAW_DIR / f"full_clean_{safe_host}.json"
        clean_paths = _artifact_paths(clean_label, trajectory_subdir="clean")
        clean_trajectory_ready = (
            not write_trajectory
            or (clean_paths["trajectory"].exists() and clean_paths["log"].exists())
        )
        if raw_path.exists() and clean_trajectory_ready and not args.force:
        ...
        clean_rows.append({
            ...
            "runtime_s": runtime_s,
            "trajectory_path": str(clean_paths["trajectory"]),
            "trajectory_log_path": str(clean_paths["log"]),
        })

    gas_results = {}
    gas_rows = []
    for adsorbate in ADSORBATE_BUILDERS:
        gas_label = f"full_gas_{adsorbate}"
        raw_path = RAW_DIR / f"full_gas_{adsorbate}.json"
        gas_paths = _artifact_paths(gas_label, trajectory_subdir="gas")
        gas_trajectory_ready = (
            not write_trajectory
            or (gas_paths["trajectory"].exists() and gas_paths["log"].exists())
        )
        if raw_path.exists() and gas_trajectory_ready and not args.force:
        ...
        gas_rows.append({
            ...
            "max_force_eV_A": _max_force(result),
            "runtime_s": runtime_s,
            "trajectory_path": str(gas_paths["trajectory"]),
            "trajectory_log_path": str(gas_paths["log"]),
        })
    ...
    for host in HOST_BUILDERS:
        for adsorbate in ADSORBATE_BUILDERS:
            ...
            if pair_csv.exists() and not args.force:
                cached_pair = pd.read_csv(pair_csv)
                cached_has_trajectories = (
                    not write_trajectory
                    or (
                        "trajectory_path" in cached_pair.columns
                        and "trajectory_log_path" in cached_pair.columns
                        and cached_pair["trajectory_path"].map(lambda p: Path(str(p)).exists()).all()
                        and cached_pair["trajectory_log_path"].map(lambda p: Path(str(p)).exists()).all()
                    )
                )
                if cached_has_trajectories:
                    print(f"Skip existing pair: {adsorbate}/{host}", flush=True)
                    all_pair_dfs[pair] = cached_pair
                    continue
                print(
                    f"Recompute pair for missing trajectories: {adsorbate}/{host}",
                    flush=True,
                )
            ...
            pair_df["initial_structure_path"] = [
                str(_artifact_paths(config.label)["initial"]) for config in configs
            ]
            pair_df["relaxed_structure_path"] = [
                str(_artifact_paths(config.label)["relaxed"]) for config in configs
            ]
            pair_df["trajectory_path"] = [
                str(_artifact_paths(config.label)["trajectory"]) for config in configs
            ]
            pair_df["trajectory_log_path"] = [
                str(_artifact_paths(config.label)["log"]) for config in configs
            ]
            ...
            if not failures.empty:
                ...
                for idx, failure in failures.iterrows():
                    ...
                    rerun_paths = _artifact_paths(
                        f"full_rerun_{_safe(config.label)}",
                        trajectory_subdir="reruns",
                    )
                    pair_df.loc[idx, "initial_attempt_trajectory_path"] = pair_df.loc[
                        idx, "trajectory_path"
                    ]
                    pair_df.loc[idx, "initial_attempt_trajectory_log_path"] = pair_df.loc[
                        idx, "trajectory_log_path"
                    ]
                    pair_df.loc[idx, "trajectory_path"] = str(rerun_paths["trajectory"])
                    pair_df.loc[idx, "trajectory_log_path"] = str(rerun_paths["log"])
    ...
    metadata = {
        ...
        "toolkit_require_d3bj": False,
        "chunk_size": args.chunk_size,
        "write_trajectories": write_trajectory,
