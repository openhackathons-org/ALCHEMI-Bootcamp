...
def _find_pair_summary_path(surface_screen_paths: Mapping[str, object] | None = None) -> Path:
...
def _coherent_adsorbate_positions_in_cell(original_atoms, wrapped_atoms, adsorbate: str):
    ...
    if ads_count <= 0 or ads_count >= len(original_atoms) or original_atoms.cell.rank < 2:
    ...
    positions[ads_start:] = original_atoms.positions[ads_start:] + best_shift @ original_atoms.cell.array
    return positions
...
def _ranked_winner_items(winners_df: pd.DataFrame, pair_summary_path: Path) -> tuple[list[dict], Path]:
    ...
    if not relaxed_dir.exists():
        raise RuntimeError(f"Cannot load final structures; missing directory: {relaxed_dir}")
    ...
    for _, row in winners_df.iterrows():
        ...
        relaxation_status = _format_relaxation_status(details, row["best_optimizer_nsteps"])
        title = f"{adsorbate}/{host}: {float(row['best_E_ads_eV']):.2f} eV"
        caption = (
            f"start {label}; final site {row['best_final_site']}; "
            f"{relaxation_status}"
        )
        items.append(
            {
...
def _display_widget_grid(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError("ipywidgets is required for the OVITO widget display.") from exc
