...
def surface_fingerprint_table(
    ...
    slab_specs: list[SurfaceScreenSlabSpec] | tuple[SurfaceScreenSlabSpec, ...] | None = None,
) -> pd.DataFrame:
...
def summarize_surface_screen_pairs(
    ...
    *,
    ...
    for (host, adsorbate), group in df.groupby(["host", "adsorbate"], sort=False):
        ...
        if eligible.empty:
            eligible = reliable
        winner = eligible.loc[eligible["E_ads_eV"].idxmin()] if len(eligible) else group.loc[group["E_ads_eV"].idxmin()]
        rows.append(
            {
    ...
    if not summary.empty:
        summary["rank_within_adsorbate"] = summary.groupby("adsorbate")[
            "best_E_ads_eV"
...
def build_application_heatmap(
    ...
    *,
    ...
    methanol_by_host = {
        row["host"]: float(row["best_E_ads_eV"])
        for _, row in pair_summary_df[pair_summary_df["adsorbate"].eq("CH3OH")].iterrows()
    }
...
def write_surface_screen_result_artifacts(
    *,
        ...
        clean_rows.append(
            {
                ...
                "converged": bool(getattr(result, "converged", False)) if result is not None else None,
    ...
    pd.DataFrame(surface_fingerprint_rows).to_csv(paths["surface_fingerprints_csv"], index=False)
