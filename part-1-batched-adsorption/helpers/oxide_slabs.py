...
* TiN rocksalt (Fm-3m, #225) - (001), (110), and (210) surfaces
...
def build_rocksalt_tin_bulk() -> pymatgen.core.Structure:
    """Build TiN rocksalt bulk (Fm-3m, #225).

    A room-temperature lattice constant near 4.24 A is common for
    stoichiometric TiN. The exact value is not used as a strict reference in
    this tutorial; it defines a reproducible ceramic surface screen.
    """
    from pymatgen.core import Lattice, Structure
...
def build_tio2_slab(
    ...
    """Build a rutile TiO2 slab for a selected Miller index."""
...
def build_tio2_100_slab(
    min_slab_size: float = 8.0,
    min_vacuum_size: float = 15.0,
    supercell: tuple[int, int, int] = (2, 2, 1),
) -> ase.Atoms:
    """Build a rutile TiO2 (100) slab."""
    return build_tio2_slab(
        (1, 0, 0),
        min_slab_size=min_slab_size,
        min_vacuum_size=min_vacuum_size,
...
def build_tio2_101_slab(
    min_slab_size: float = 8.0,
    min_vacuum_size: float = 15.0,
    supercell: tuple[int, int, int] = (2, 2, 1),
) -> ase.Atoms:
    """Build a rutile TiO2 (101) slab."""
    return build_tio2_slab(
        (1, 0, 1),
        min_slab_size=min_slab_size,
        min_vacuum_size=min_vacuum_size,
...
def build_tin_slab(
    ...
    """Build a rocksalt TiN slab for a selected Miller index."""
    bulk = build_rocksalt_tin_bulk()
    return build_slab(
        bulk,
        miller_index=miller_index,
        min_slab_size=min_slab_size,
...
def build_tin_001_slab(
    min_slab_size: float = 8.0,
    min_vacuum_size: float = 15.0,
    supercell: tuple[int, int, int] = (2, 2, 1),
) -> ase.Atoms:
    """Build a rocksalt TiN (001) slab."""
    return build_tin_slab(
        (0, 0, 1),
        min_slab_size=min_slab_size,
        min_vacuum_size=min_vacuum_size,
        supercell=supercell,
...
def build_tin_110_slab(
    min_slab_size: float = 8.0,
    min_vacuum_size: float = 15.0,
    supercell: tuple[int, int, int] = (2, 2, 1),
) -> ase.Atoms:
    """Build a rocksalt TiN (110) slab."""
    return build_tin_slab(
        (1, 1, 0),
        min_slab_size=min_slab_size,
        min_vacuum_size=min_vacuum_size,
...
def build_tin_210_slab(
    min_slab_size: float = 8.0,
    min_vacuum_size: float = 15.0,
    supercell: tuple[int, int, int] = (2, 2, 1),
) -> ase.Atoms:
    """Build a rocksalt TiN (210) stepped/high-index slab."""
    return build_tin_slab(
        (2, 1, 0),
        min_slab_size=min_slab_size,
        min_vacuum_size=min_vacuum_size,
