...
def test_oc20dense_full_data_root_extends_mapping_lookup(tmp_path, monkeypatch):
    ...
    assert (
        common.oc20dense_mapping_file(local_root, "oc20dense_mapping.pkl") == expected
    )
...
def test_oc20dense_model_specific_outputs_are_present():
    ...
    summary = _csv_rows(root / "tables" / "system_summary.csv")
    dft_final = _csv_rows(
        root / "dft_final_single_points" / "tables" / "dft_final_sp_system_summary.csv"
    )
...
def test_oc20dense_mace_adsorption_energy_subtraction_round_trips():
    root = _require_oc20dense_artifacts()
    eads = _csv_rows(
        root / "mace_adsorption_energy" / "tables" / "mace_adsorption_energies.csv"
    )
...
def test_main_notebook_keeps_oc20dense_validation_inside_tutorial_flow():
    notebook = json.loads(
        (PART1 / "alchemi-mace-adsorption-search.ipynb").read_text(encoding="utf-8")
    )
    source = "\n".join(
