...
def _write_report(
    *,
    ...
    e_col = adsorption_energy_column(diagnostic_df)
    winner = reliable.loc[reliable[e_col].idxmin()]
    top_c = reliable[
        (reliable["final_site"] == "top")
        & (reliable["binding_atom_symbol"] == "C")
    ]
    top_best = top_c.loc[top_c[e_col].idxmin()] if not top_c.empty else None
    ...
    if prior_batch_df is not None and not prior_batch_df.empty:
        ...
        if not prior_reliable.empty:
            prior_e_col = adsorption_energy_column(prior_reliable)
            batch_min = prior_reliable.loc[prior_reliable[prior_e_col].idxmin()]
    ...
    lines = [
        ...
        "",
        "The diagnostic uses native Toolkit MACE with D3(BJ) disabled to match "
        "the notebook's OC20Dense reference convention.",
        "",
        ...
        f"- 200-step MACE-MPA-0 minimum: {float(winner[e_col]):.6f} eV, "
    ...
    if top_best is not None:
        lines.append(
            f"- Best final top C-down structure: {float(top_best[e_col]):.6f} eV, "
            f"steps={int(top_best['optimizer_nsteps'])}."
        )
    if batch_min is not None:
        lines.append(
            f"- Corrected notebook batch minimum ({notebook_steps_text}): "
            f"{float(batch_min[adsorption_energy_column(batch_min.to_frame().T)]):.6f} eV, "
    ...
    lines.extend([
        ...
        "",
        f"- Context reference in the tutorial: {ref.e_ads_ev:.3f} eV, site={ref.binding_site}, "
        f"scope={ref.reference_scope}, source={ref.ref}.",
        f"- Difference from context reference for the 200-step minimum: "
        f"{float(winner[e_col]) - float(ref.e_ads_ev):+.3f} eV "
        f"({abs(float(winner[e_col]) - float(ref.e_ads_ev)) / LITERATURE_OC157_MAD_GUIDE_EV:.2f} x "
        f"the literature OC157 relative-energy MAD guide of {LITERATURE_OC157_MAD_GUIDE_EV:.2f} eV).",
        "- Site agreement: failed for the MACE-MPA-0 minimum because it relaxes to an fcc hollow; "
        ...
        "functional, dispersion convention, and backend settings.",
        "",
        ...
        "preferred top site. The current MACE-MPA-0 result reproduces that qualitative warning.",
        "",
        "## Per-Start Summary",
        "",
