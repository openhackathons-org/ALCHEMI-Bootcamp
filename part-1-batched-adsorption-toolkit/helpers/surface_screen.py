...
def summarize_reference_relaxations(
    clean_opts, gas_opts, host_names, adsorbates, *, atomic_data_to_ase
):
    """Extract clean-slab and gas-reference energies/structures from relaxation outputs."""
    E_HOST, HOST_RELAXED, clean_rows = {}, {}, []
    for name, opt in zip(host_names, clean_opts):
        ...
        clean_rows.append({
            "surface": name, "atoms": len(relaxed), "converged": opt.converged,
            "steps": opt.num_optimization_steps, "max |F| (eV/A)": round(fmax, 4),
            "E_clean_slab (eV)": round(E_HOST[name], 4),
        })
    E_ADS_GAS, GAS_RELAXED, gas_rows = {}, {}, []
    for name, opt in zip(adsorbates, gas_opts):
        relaxed = atomic_data_to_ase(opt)
        GAS_RELAXED[name] = relaxed
        E_ADS_GAS[name] = float(opt.energy)
        gas_rows.append({
            "adsorbate": name, "converged": opt.converged,
            "steps": opt.num_optimization_steps, "E_gas (eV)": round(E_ADS_GAS[name], 4),
        })
    return {
        "E_HOST": E_HOST, "HOST_RELAXED": HOST_RELAXED, "E_ADS_GAS": E_ADS_GAS,
        "GAS_RELAXED": GAS_RELAXED, "clean_df": pd.DataFrame(clean_rows),
        "gas_df": pd.DataFrame(gas_rows),
    }


def load_reference_relaxations(paths, host_names, adsorbates, *, relpath_fn, ase_read):
    """Load saved clean-slab and gas-reference energies/structures from CSVs."""
    clean_df = pd.read_csv(
        require_surface_screen_artifact(paths, "clean_slab_energies_csv", relpath=relpath_fn)
    )
    gas_df = pd.read_csv(
        require_surface_screen_artifact(paths, "gas_energies_csv", relpath=relpath_fn)
    )
    E_HOST, HOST_RELAXED = {}, {}
    for _, row in clean_df.iterrows():
        E_HOST[row["host"]] = float(row["energy_eV"])
        HOST_RELAXED[row["host"]] = ase_read(row["structure_path"])
    E_ADS_GAS = {row["adsorbate"]: float(row["energy_eV"]) for _, row in gas_df.iterrows()}
    clean_cols = [c for c in ["host", "converged", "optimizer_nsteps", "max_force_eV_A", "energy_eV"] if c in clean_df]
    gas_cols = [c for c in ["adsorbate", "converged", "optimizer_nsteps", "max_force_eV_A", "energy_eV"] if c in gas_df]
    return {
        "E_HOST": E_HOST, "HOST_RELAXED": HOST_RELAXED, "E_ADS_GAS": E_ADS_GAS,
        "clean_df": clean_df[clean_cols], "gas_df": gas_df[gas_cols],
    }


def build_site_candidates(host_relaxed, host_names, site_finders, site_classes):
    """Find adsorption-site candidates on each relaxed slab and validate coverage."""
    candidates = {host: site_finders[host](host_relaxed[host]) for host in host_names}
    missing = []
    for host, names in site_classes.items():
        avail = candidates[host]
        missing.extend((host, s) for s in names if s not in avail or not avail[s])
    if missing:
        raise RuntimeError(f"Site finder did not produce requested site classes: {missing}")
    count_df = pd.DataFrame([
        {"surface": host,
         "candidate positions per class": ", ".join(
             f"{s}:{len(candidates[host][s])}" for s in site_classes[host])}
        for host in host_names
    ])
    return candidates, count_df


def pack_adsorption_batches(pairs, grid, batch_size):
    """Pack six-start adsorbate/surface questions into Toolkit batches <= batch_size."""
    batches, current = [], []
    for host, adsorbate in pairs:
        group = [(host, adsorbate, cfg) for cfg in grid[(host, adsorbate)]]
...
def pair_replies_from_results(out_results, pairs, *, RelaxationBatchResult):
    """Wrap per-pair OptimizationResult lists into RelaxationBatchResult records."""
    return {
        pair: RelaxationBatchResult(
            atoms=[r.model_dump() if hasattr(r, "model_dump") else r for r in results],
            status="Success",
...
def write_surface_screen_raw(
    paths, grid, pair_replies, *, atomic_data_to_ase, safe_label, json_path_fn, ase_write
):
    """Write per-structure raw JSON + initial/relaxed extxyz for the screen."""
    paths["raw"].mkdir(parents=True, exist_ok=True)
    paths["initial_structures"].mkdir(parents=True, exist_ok=True)
    paths["relaxed_structures"].mkdir(parents=True, exist_ok=True)
    for pair, reply in pair_replies.items():
        for config, result in zip(grid[pair], reply.atoms):
            json_path_fn(paths, config.label).write_text(
                result.model_dump_json(indent=2), encoding="utf-8")
            ase_write(paths["initial_structures"] / f"{safe_label(config.label)}.extxyz", config.atoms)
            ase_write(paths["relaxed_structures"] / f"{safe_label(config.label)}.extxyz", atomic_data_to_ase(result))


def load_surface_screen_pair_replies(
    paths, pairs, grid, *, relpath_fn, json_path_fn, OptimizationResult, RelaxationBatchResult
):
    """Load saved per-structure surface-screen results into RelaxationBatchResult records."""
    require_surface_screen_artifact(paths, "adsorption_results_csv", relpath=relpath_fn)
    pair_replies, missing = {}, []
    for pair in pairs:
        results = []
        for config in grid[pair]:
            path = json_path_fn(paths, config.label)
            if not path.exists():
                missing.append(path)
                continue
            results.append(OptimizationResult.model_validate(
                json.loads(path.read_text(encoding="utf-8"))))
        pair_replies[pair] = RelaxationBatchResult(
            atoms=results, status="Success",
    ...
    if missing:
        raise RuntimeError(
            ...
            f"{relpath_fn(missing[0])}")
    return pair_replies


def build_surface_screen_outputs(
    pair_results, paths, *, adsorbate_hints, force_threshold_eV_A,
    packed_batches, loaded_from_cache, relpath_fn,
):
    """Build and persist the screen-level result tables (results/summary/heatmap/...)."""
    adsorption_results_df = pd.concat(
        [df.assign(host=host, adsorbate=adsorbate) for (host, adsorbate), df in pair_results.items()],
        ignore_index=True)
    batch_summary_df = pd.DataFrame([
        {"batch_label": f"surface_screen_adsorption_batch_{i:02d}", "batch_type": "adsorption",
         "n_structures": len(b), "loaded_from_cache": bool(loaded_from_cache)}
        for i, b in enumerate(packed_batches, start=1)])
    step_statistics_df = build_step_statistics(
        adsorption_results_df, green_step_max=200, yellow_step_max=500,
        force_threshold_eV_A=force_threshold_eV_A)
    surface_pair_summary_df = summarize_surface_screen_pairs(
        adsorption_results_df, step_statistics_df, exclude_red_step_status=True)
    application_heatmap_df = build_application_heatmap(surface_pair_summary_df, adsorbate_hints=adsorbate_hints)
    difficult_cases_df = build_difficult_cases(adsorption_results_df, step_statistics_df)
    ranked_winners_df = (
        surface_pair_summary_df.sort_values(["adsorbate", "rank_within_adsorbate", "best_E_ads_eV"])
        .groupby("adsorbate", as_index=False).head(1).sort_values("adsorbate").reset_index(drop=True))
    paths["tables"].mkdir(parents=True, exist_ok=True)
    adsorption_results_df.to_csv(paths["adsorption_results_csv"], index=False)
    batch_summary_df.to_csv(paths["batch_summary_csv"], index=False)
    step_statistics_df.to_csv(paths["step_statistics_csv"], index=False)
    ...
    difficult_cases_df.to_csv(paths["difficult_cases_csv"], index=False)
    return {
        "adsorption_results_df": adsorption_results_df, "batch_summary_df": batch_summary_df,
        "step_statistics_df": step_statistics_df, "surface_pair_summary_df": surface_pair_summary_df,
        "application_heatmap_df": application_heatmap_df, "difficult_cases_df": difficult_cases_df,
        "ranked_winners_df": ranked_winners_df, "tables_dir": paths["tables"],
    }
