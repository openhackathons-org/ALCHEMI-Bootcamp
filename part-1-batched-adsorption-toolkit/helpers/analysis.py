    ...
    if len(final_ads) > 1 and len(initial_ads) == len(final_ads):
        ...
        for i in range(len(initial_ads)):
            for j in range(i + 1, len(initial_ads)):
                d_initial = float(
                    np.linalg.norm(initial_ads.positions[i] - initial_ads.positions[j])
                )
                bond_cutoff = 1.25 * (
        ...
        for i, j, d_initial in bonded_pairs:
            d_final = float(
                np.linalg.norm(final_ads.positions[i] - final_ads.positions[j])
            )
...
def classify_final_site(
    ...
    if len(ads) == 0:
        raise ValueError(
            "final_atoms contains no adsorbate atoms after slab_atom_count"
        )
    ...
    final_site, site_dist = _nearest_site_name(
        binding_position, site_map, slab.cell.array
    )
    ...
    for (host, adsorbate), df in pair_results.items():
        ...
        rows.append(
            ValidationResult(
                ...
                E_ref_eV=round(ref.e_ads_ev, 3)
                if ref and ref.e_ads_ev is not None
                else None,
...
def strict_parity_subset(summary_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if summary_df.empty:
        return summary_df
    return summary_df[
        summary_df["reference_scope"].isin(["strict", "near-strict"])
    ].copy()
