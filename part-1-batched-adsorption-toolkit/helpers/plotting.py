...
def plot_h2o_batch_speedup(
    ...
    ax.fill_between(
        df["batch_size"], df["speedup_vs_single"], color=nv_green, alpha=0.16
    )
    ...
    fig.suptitle(
        "ALCHEMI Toolkit batched H$_2$O relaxation", color=light, fontsize=16, y=0.98
    )
...
def plot_adsorption_batch_sweep(
    ...
    *,
    ...
    if "status" not in df:
        df["status"] = "ok"
    if "gpu_free_drop_gb" not in df and {
        "gpu_free_before_gb",
        "gpu_free_after_gb",
    }.issubset(df.columns):
        df["gpu_free_drop_gb"] = (
            df["gpu_free_before_gb"] - df["gpu_free_after_gb"]
        ).clip(lower=0.0)
    ...
    if len(oom):
        for _, row in oom.iterrows():
            axes[1].scatter(
                ...
                linewidths=2,
                label="OOM"
                if "OOM" not in axes[1].get_legend_handles_labels()[1]
                else None,
            )
    ...
    if (
        "process_peak_reserved_gb" in ok
        and ok["process_peak_reserved_gb"].notna().any()
    ):
...
def plot_device_throughput_comparison(
    results,
    output_path: str,
    *,
    title: str = "CPU vs GPU: batching amortizes per-call overhead",
    caption: str | None = None,
) -> str:
    """Plot CPU vs GPU throughput (structures/s) vs batch size, one panel per system.

    ``results`` maps a system label to ``{"cpu": df, "gpu": df_or_None,
    "atoms_per_structure": int}``. Each panel overlays the CPU and GPU curves and
    annotates the crossover batch size where the GPU first overtakes the CPU.
    """
    import matplotlib.pyplot as plt
    ...
    n = max(1, len(labels))
    fig, axes = plt.subplots(1, n, figsize=(6.1 * n, 5.0), facecolor=dark, squeeze=False)
    axes = axes[0]

    def _draw(ax, cpu_df, gpu_df, label, atoms):
        ax.set_facecolor(dark)

        def _line(df, color, lab, ls):
            if df is None or len(df) == 0:
                return
            df = df.sort_values("batch_size")
            ax.plot(
                df["batch_size"], df["structures_per_s"], color=color, marker="o",
                linewidth=2.6, markersize=6, linestyle=ls, label=lab,
            )

        _line(cpu_df, muted, "CPU", "--")
        _line(gpu_df, nv_green, "GPU", "-")

        if gpu_df is not None and len(gpu_df) and cpu_df is not None and len(cpu_df):
            merged = pd.merge(
                cpu_df[["batch_size", "structures_per_s"]],
                gpu_df[["batch_size", "structures_per_s"]],
                on="batch_size", suffixes=("_cpu", "_gpu"),
            ).sort_values("batch_size")
            crossed = merged[merged["structures_per_s_gpu"] >= merged["structures_per_s_cpu"]]
            if len(crossed):
                row = crossed.iloc[0]
                ax.axvline(row["batch_size"], color=nv_blue, linestyle=":", linewidth=1.5)
                ax.annotate(
                    f"GPU>CPU @ batch {int(row['batch_size'])}",
                    xy=(row["batch_size"], row["structures_per_s_gpu"]),
                    xytext=(6, -26), textcoords="offset points", color=nv_blue, fontsize=9,
                )
        ...
        all_batches = sorted(
            {
                ...
                for x in df["batch_size"]
            }
        )
        if all_batches:
            ax.set_xscale("log", base=2)
            ax.set_xticks(all_batches)
            ax.set_xticklabels([str(x) for x in all_batches])
        ax.set_xlabel("structures in one batch", color=light, fontsize=11)
        ax.set_ylabel("throughput (structures/s)", color=light, fontsize=11)
        ax.set_title(f"{label}  (~{atoms} atoms each)", color=light, fontsize=12, pad=8)
        ax.tick_params(colors=light, labelsize=10)
        for spine in ax.spines.values():
            spine.set_color("#4B5563")
        ax.grid(True, color="#2F3A44", linewidth=0.9, alpha=0.75)
    ...
    for ax, label in zip(axes, labels):
        d = results[label]
        _draw(ax, d.get("cpu"), d.get("gpu"), label, d.get("atoms_per_structure", "?"))

    fig.suptitle(title, color=light, fontsize=15, y=0.99)
    if caption:
        fig.text(0.06, 0.005, caption, color=muted, fontsize=8.5)
        fig.tight_layout(rect=(0.0, 0.04, 1.0, 0.95))
    else:
        fig.tight_layout(rect=(0.0, 0.0, 1.0, 0.95))
    output_path = _save_figure(
        fig, output_path, dpi=300, bbox_inches="tight", facecolor=fig.get_facecolor()
    )
    plt.close(fig)
    return output_path
...
def plot_surface_screen_heatmap(
    ...
    *,
    ...
    ax.set_yticklabels(
        [
            label.replace("H2O", "H$_2$O")
            .replace("NH3", "NH$_3$")
            .replace("CH3OH", "CH$_3$OH")
...
def plot_adsorption_energy_spread(
    ...
    fig, axes = plt.subplots(
        n_pairs, 1, figsize=(9, max(2.5 * n_pairs, 3)), sharex=False
    )
    if n_pairs == 1:
        axes = [axes]
    site_markers = {
...
def plot_site_agreement_heatmap(
    ...
    for i, host in enumerate(host_names):
        for j, adsorbate in enumerate(adsorbates):
            row = site_df[
                (site_df["host"] == host) & (site_df["adsorbate"] == adsorbate)
            ]
            if len(row):
...
def plot_adsorption_summary(
    ...
    ax.set_yticks(y)
    ax.set_yticklabels(
        [f"{row['pair']} [{row['status']}]" for _, row in disc_df.iterrows()]
    )
    ax.set_xlabel("E_ads (eV) - more negative = stronger binding")
    ax.set_title(
