...
class ToolkitBackend:
    ...
    def _select_mace_head(self, mace: object, head: str) -> None:
        ...
        def adapt_input_with_head(
            wrapper_self: object, data: object, **kwargs: object
        ) -> dict[str, object]:
    ...
    def _to_atomic_data(self, payload: AtomicStructurePayload) -> object:
        ...
        if payload.active_mask is not None:
            ...
            categories = torch.zeros(
                len(payload.numbers), dtype=torch.long, device=self.device
            )
    ...
    def _to_result(
        ...
        *,
        ...
        return OptimizationResult(
            ...
            stress=stress_tensor.flatten().tolist()
...
def run_toolkit_relaxation_with_trajectory(
    ...
    *,
    ...
    if backend.config.use_cached_responses:
        raise RuntimeError(
            "Trajectory capture requires a fresh Toolkit run, not cache replay."
        )
    if not atoms_list:
        raise ValueError("Cannot relax an empty batch.")
    ...
    sink = HostMemory(capacity=max(1, backend.config.toolkit_n_steps) * len(atoms_list))
    ...
    if len(sink) > 0:
        ...
        for structure_index, original in enumerate(atoms_list):
            ...
            for frame_index in range(structure_index, len(snapshots), expected):
                ...
                force_norm = (
                    np.linalg.norm(forces, axis=1) if len(forces) else np.array([])
                )
