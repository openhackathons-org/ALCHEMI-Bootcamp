"""Download and verify shared model assets for the five core notebooks."""
...
def ensure_pinned_asset(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix=f".{path.stem}-", dir=path.parent) as staging:
...
def main() -> None:
    ...
    model = PINS["model"]
    path = Path(get_model_path(model["checkpoint_alias"])).resolve()
    actual = sha256_file(path)
    ...
    configured_d3_path = os.environ.get("ALCHEMI_D3_PARAM_FILE")
    ...
    d3_path = ensure_pinned_asset(
        ...
        label="D3 parameters",
        create=generate_d3_parameter_file,
    )
