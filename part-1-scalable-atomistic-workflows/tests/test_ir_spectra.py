...
from aux.spectra import band_centroid, welch_dipole_current_spectrum  # noqa: E402
...
def test_tutorial_production_settings_give_two_five_ps_windows() -> None:
    wavenumber, _, per_segment = welch_dipole_current_spectrum(
        np.zeros((20_000, 3)),
        0.5,
        segment_time_fs=5_000.0,
        overlap=0.5,
    )

    assert per_segment.shape == (2, 5_001)
    assert wavenumber[1] - wavenumber[0] == pytest.approx(6.6712819)
...
def test_welch_methodology_settings_are_required() -> None:
    parameters = inspect.signature(welch_dipole_current_spectrum).parameters

    assert parameters["segment_time_fs"].default is inspect.Parameter.empty
    assert parameters["overlap"].default is inspect.Parameter.empty
