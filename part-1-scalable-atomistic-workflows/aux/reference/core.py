...
topology, and isotope checks. No pickled arrays are accepted.
...
def load_psi4_b973c_ir_artifact(
    ...
    *,
    ...
    if validation.get("status") != "passed" or validation.get(
        ...
        raise IRArtifactError("reference artifact is not ready for comparison")
    ...
    if (
        ...
        raise IRArtifactError("reference artifact failed its gradient check")
    try:
        antisymmetry = float(
    ...
    if (
        ...
        raise IRArtifactError("reference artifact failed its Hessian-consistency check")
    for field in (
