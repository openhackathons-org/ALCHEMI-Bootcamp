...
StageState = Literal[
    "ready", "active", "complete", "not_reported", "withheld", "action"
]
CalloutKind = Literal["before", "check", "result", "note", "boundary"]
ResultState = Literal[
    "observed", "pass", "not_reported", "withheld", "action"
]
...
_STAGE_STYLES: dict[str, _SemanticStyle] = {
    ...
    "not_reported": _SemanticStyle("NOT REPORTED", "#9A3412", "#FFEDD5"),
    # Compatibility for notebooks created before the learner-facing rename.
    "withheld": _SemanticStyle("NOT REPORTED", "#9A3412", "#FFEDD5"),
...
_CALLOUT_STYLES: dict[str, _SemanticStyle] = {
    ...
    "check": _SemanticStyle("WHAT TO CHECK", "#854D0E", "#FEF9C3"),
    "note": _SemanticStyle("NOTE", "#9A3412", "#FFEDD5"),
    # Compatibility alias. New notebook source uses ``kind="note"``.
    "boundary": _SemanticStyle("NOTE", "#9A3412", "#FFEDD5"),
}

_RESULT_STYLES: dict[str, _SemanticStyle] = {
    "observed": _SemanticStyle("RESULT", "#374151", "#F3F4F6"),
    "pass": _SemanticStyle("CHECK PASSED", "#365314", "#ECFCCB"),
    "not_reported": _SemanticStyle("NOT REPORTED", "#9A3412", "#FFEDD5"),
    # Compatibility for notebooks created before the learner-facing rename.
    "withheld": _SemanticStyle("NOT REPORTED", "#9A3412", "#FFEDD5"),
    "action": _SemanticStyle("NEEDS ATTENTION", "#991B1B", "#FEE2E2"),
}
...
def stage_card_html(
    *,
    ...
    particular stage is ready, active, complete, not reported, or needs action.
    ``compute_time`` optionally adds a short learner-facing time estimate or
    measured section time, such as ``"about 20 seconds on one H100"``.
    """
    ...
    if compute_time is not None:
        compute_time = str(compute_time).strip()
        if not compute_time:
            raise ValueError("compute_time cannot be empty")
        timing_html = (
            f'<div style="color:{_MUTED};font-size:12px;line-height:1.45;'
            f'margin-top:7px;"><strong style="color:{_TEXT};">'
            f'Compute time:</strong> {_escaped_text(compute_time)}</div>'
        )

    return (
        f'<section class="alchemi-ui alchemi-stage-card" role="region" aria-labelledby="{heading_id}" '
        ...
        f'{_escaped_text(outcome)}</div>{timing_html}</div></section>'
    )


def stage_card(**kwargs: Any) -> HTML:
...
def callout_html(
    ...
    *,
    ...
    ``RESULT``, ``CHECK PASSED``, ``NOT REPORTED``, or ``NEEDS ATTENTION``.
    """
    ...
    return (
        f'<aside class="alchemi-ui alchemi-callout" role="note" '
        f'aria-label="{_escaped_attribute(style.label)}" '
        ...
        f'font-family:{_FONT_STACK};box-sizing:border-box;">'
...
def callout(body: str, **kwargs: Any) -> HTML:
...
def notebook_hero_html(
    *,
    ...
    return (
        '<style>'
        '.alchemi-ui,.alchemi-ui *{box-sizing:border-box;}'
        '.alchemi-ui{overflow-wrap:anywhere;}'
        '.alchemi-ui code{white-space:normal;overflow-wrap:anywhere;}'
        f'.text_cell_render,.jp-MarkdownCell .jp-InputArea,'
        f'.markdown-cell-renderer,.rendered-markdown{{max-width:{_CONTENT_WIDTH_PX}px;}}'
        f'.jp-MarkdownOutput{{max-width:{_CONTENT_WIDTH_PX}px;'
        'width:100%!important;overflow-x:auto;}}'
        '@media (max-width:899px){'
        '.alchemi-lesson-summary{grid-template-columns:1fr!important;}'
        '}'
        '@media (max-width:599px){'
        '.alchemi-hero{aspect-ratio:auto!important;min-height:340px;}'
        '.alchemi-hero-copy{width:72%!important;padding:22px 20px!important;}'
        '.alchemi-hero-copy h1{font-size:28px!important;}'
        '.alchemi-hero-copy p{font-size:13px!important;}'
        '}'
        '</style>'
        f'<section class="alchemi-ui alchemi-hero" role="region" '
        f'aria-labelledby="alchemi-notebook-title" '
        ...
        '</div><div class="alchemi-hero-copy" '
        'style="position:relative;z-index:1;width:55%;height:100%;'
...
def lesson_summary_html(*, do: str, learn: str, need: str) -> str:
    ...
    return (
        f'<dl class="alchemi-ui alchemi-lesson-summary" '
        f'aria-label="Lesson summary" style="max-width:{_CONTENT_WIDTH_PX}px;'
        f'margin:14px 0;display:grid;grid-template-columns:repeat(auto-fit,minmax('
        f'220px,1fr));gap:10px;font-family:{_FONT_STACK};box-sizing:border-box;">'
        + "".join(cards)
        + "</dl>"
    )
...
def figure_placeholder_html(*, title: str, description: str) -> str:
    ...
    return (
        f'<div class="alchemi-ui alchemi-figure-placeholder" role="img" '
        f'aria-label="{aria}" style="max-width:{_CONTENT_WIDTH_PX}px;'
        ...
        f'background:#F9FAFB;font-family:{_FONT_STACK};box-sizing:border-box;">'
...
def process_diagram_html(
    *,
    title: str,
    steps: Sequence[str],
    caption: str,
) -> str:
    """Return a compact, accessible left-to-right process diagram.

    The diagram uses only escaped HTML and inline CSS so the same output
    renders offline in JupyterLab and VS Code. Numbered steps show the order;
    the complete relationship is repeated in ``aria-label`` and ``figcaption``.
    """

    if isinstance(steps, (str, bytes)):
        raise TypeError("steps must be a sequence of labels, not one string")
    labels = tuple(str(step).strip() for step in steps)
    if len(labels) < 2:
        raise ValueError("a process diagram requires at least two steps")
    if any(not label for label in labels):
        raise ValueError("process diagram step labels cannot be empty")

    relationship = " then ".join(labels)
    aria = _escaped_attribute(
        f"Process diagram: {title}. {relationship}. {caption}"
    )
    step_width_px = 124 if len(labels) == 5 else 148
    items: list[str] = []
    for index, label in enumerate(labels, start=1):
        items.append(
            f'<div style="min-width:{step_width_px}px;'
            f'flex:1 1 {step_width_px}px;display:flex;'
            'align-items:stretch;">'
            '<div style="min-width:0;flex:1 1 auto;padding:11px 12px;'
            f'border:1px solid {_BORDER};border-top:3px solid {_NVIDIA_GREEN};'
            f'border-radius:9px;background:{_SURFACE};">'
            f'<div style="color:{_MUTED};font-size:10px;font-weight:800;'
            f'letter-spacing:0.08em;margin-bottom:4px;">STEP {index}</div>'
            f'<div style="color:{_TEXT};font-size:12px;font-weight:700;'
            f'line-height:1.4;">{_escaped_text(label)}</div></div></div>'
        )

    return (
        f'<figure class="alchemi-ui alchemi-process" role="img" '
        f'aria-label="{aria}" style="max-width:'
        f'{_CONTENT_WIDTH_PX}px;margin:14px 0;padding:16px 17px 14px;'
        ...
        + "".join(items)
        + '</div><figcaption style="color:'
        + _MUTED
        + ';font-size:12px;line-height:1.45;margin-top:10px;">'
        + _escaped_text(caption)
        + "</figcaption></figure>"
    )
...
def figure_with_alt(
    ...
    *,
    ...
    """Return a responsive PNG figure with descriptive alternative text.

    The default nbconvert Lab template does not currently carry Matplotlib's
    PNG metadata into the exported ``<img>`` element.  Embedding the PNG in a
    small HTML wrapper keeps the same offline notebook output while making the
    description available in JupyterLab, VS Code, and exported HTML.
    """
    ...
    payload = BytesIO()
    figure.savefig(payload, format="png", dpi=dpi, bbox_inches="tight")
    encoded = base64.b64encode(payload.getvalue()).decode("ascii")

    from IPython.display import HTML

    return HTML(
        f'<figure style="max-width:{_CONTENT_WIDTH_PX}px;margin:14px 0;">'
        f'<img src="data:image/png;base64,{encoded}" '
        f'alt="{_escaped_attribute(alt_text)}" '
        'style="display:block;max-width:100%;height:auto;">'
        "</figure>"
    )


def readable_table_html(
    table: pd.DataFrame,
    *,
    label: str,
    ...
    """Return a complete, wrapped DataFrame table for notebook display.

    Pandas' default rich representation shortens long strings. That is useful
    in a console but can hide the reason a tutorial result was not reported.
    This helper renders every cell in full, wraps long text within the shared
    notebook width, and lets the caller choose a plain label for missing data.
    """
    ...
    if not isinstance(show_index, bool):
        raise TypeError("show_index must be a bool")

    table_markup = table.to_html(
        border=0,
        classes=("alchemi-dataframe",),
        escape=True,
        index=show_index,
        na_rep=str(missing),
    )
    return (
        f'<div class="alchemi-ui alchemi-table" role="region" '
        f'aria-label="{_escaped_attribute(label)}" '
        f'style="max-width:{_CONTENT_WIDTH_PX}px;margin:12px 0;overflow-x:auto;'
        f'font-family:{_FONT_STACK};box-sizing:border-box;">'
        "<style>"
        ".alchemi-dataframe{width:100%;border-collapse:collapse;"
        f"color:{_TEXT};font-size:12px;line-height:1.45;background:{_SURFACE};}}"
        ".alchemi-dataframe th,.alchemi-dataframe td{padding:8px 9px;"
        f"border:1px solid {_BORDER};vertical-align:top;text-align:left;"
        "white-space:normal;overflow-wrap:anywhere;}"
        f".alchemi-dataframe thead th{{background:#F3F4F6;color:{_TEXT};"
        "font-weight:750;}"
        ".alchemi-dataframe tbody th{background:#F9FAFB;font-weight:650;}"
        "</style>"
        f"{table_markup}</div>"
    )


def readable_table(table: pd.DataFrame, **kwargs: Any) -> HTML:
    """Return an ``IPython.display.HTML`` complete-width DataFrame table."""

    from IPython.display import HTML

    return HTML(readable_table_html(table, **kwargs))
...
def notebook_progress_html(**kwargs: Any) -> HTML:
...
class NotebookProgress:
    ...
    The class prefers an IPython display handle, which stores ordinary HTML in
    the notebook and therefore survives static export.  It falls back to an
    ``ipywidgets.HTML`` value when a display handle is unavailable.
    ``auto_display=False`` keeps the object side-effect free for scripts and
    tests; call :meth:`show` later if a notebook display is wanted.
    """
    ...
    def _display(self, value: Any, *, display_id: bool | None = None) -> Any:
        """Display one value through IPython's notebook display function."""
