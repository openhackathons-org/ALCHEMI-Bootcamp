...
class PredictedChargeIRHook:
    ...
    def __init__(
        ...
        *,
        ...
        if warmup_status == production_status:
            raise ValueError("warmup_status and production_status must differ")
        if not np.isfinite(charge_tolerance) or charge_tolerance < 0.0:
            raise ValueError("charge_tolerance must be non-negative and finite")
        self.warmup_steps = int(warmup_steps)
        self.n_steps = int(n_steps)
        self.dt_fs = float(dt_fs)
        self.warmup_status = int(warmup_status)
    ...
    def stage_counts(self) -> dict[str, int]:
        """Observed fused-stage hook calls, split at the NVT/NVE transition."""

        return {
            f"status_{self.warmup_status}_warmup_steps": self._warmup_calls,
            f"status_{self.production_status}_production_steps": (
                self._production_calls
            ),
        }

    def _record_stage_route(self, batch: Batch, expected_status: int) -> torch.Tensor:
        ...
        if hasattr(batch, "nvt_steps_done") and hasattr(batch, "nve_steps_done"):
            nvt_steps = batch.nvt_steps_done.reshape(-1)
            nve_steps = batch.nve_steps_done.reshape(-1)
            if expected_status == self.warmup_status:
                mismatch = torch.any(
                    (nvt_steps != self._warmup_calls) | (nve_steps != 0)
                )
            elif expected_status == self.production_status:
                mismatch = torch.any(
                    (nvt_steps != self.warmup_steps)
                    | (nve_steps != self._production_calls)
                )
            else:
                raise ValueError(f"Unexpected fused-stage status {expected_status}")
        else:
    ...
    def __call__(self, ctx: DynamicsContext, stage: DynamicsStage) -> None:
        ...
        if batch is None:
            return
        # Stage-entry counters identify the integrator that performed this
        # update even when its convergence hook has already advanced status.
        # The first NVE frame has ctx.step_count == warmup_steps.
        if ctx.step_count < self.warmup_steps:
            self._warmup_calls += 1
            mismatch = self._record_stage_route(
                batch, expected_status=self.warmup_status
            )
